...
import { DataJson, ExpandMethod, ResGroupType, SplitResInfoOptions, ValidationPathType } from '@/types';
...
export function execute() {
    return async function(uri:vscode.Uri) {
        const packagePath = await fileUtils.validatePath(uri, ValidationPathType.folder, /(\.package)$/i);
        const dataJson:DataJson = readDataJson(packagePath);
        ...
        const atlasImagesGlob = new vscode.RelativePattern(imagesPath, `**/${atlasFileNoExt}_[0-9][0-9].*`);
        const files = await findFiles(atlasImagesGlob);
        const atlasImagesRegex = new RegExp(`${atlasFileNoExt}_\\d{2}\\.png$`, 'i');

        const atlasImages = files.filter(file => {

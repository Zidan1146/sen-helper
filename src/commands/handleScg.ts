import { runSenAndExecute, validateSenPath } from "../utils/senUtils";
import * as vscode from 'vscode';
import * as fs from 'fs';
import { isEncodeWithSplitLabel, PathType, validatePath, writeSplitLabelIntoJson } from "../utils/fileUtils";

export const decodeScg = vscode.commands.registerCommand('sen-helper.scg.decode', async (uri: vscode.Uri) => {
...
export const encodeScg = vscode.commands.registerCommand('sen-helper.scg.encode', async (uri: vscode.Uri) => {
    ...
    const isSplitLabel = isEncodeWithSplitLabel(packagePath).toString();
